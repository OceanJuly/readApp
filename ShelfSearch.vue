...
<script>
import { getBookShelf, setLocalStorage } from '../../utils/localStorage'
    ...
    search () {
      ...
      this.shelfList.forEach(book => {
        if (book.type === 1) {
          ...
          book.itemList.forEach(bookItem => {
            target.push(bookItem)
          })
        }
      })
      this.setShelfList(target.filter(ele => ele.fileName.indexOf(this.searchText) !== -1))
    },
    onTabClick (id) {
      this.selectedTab = id
    },
