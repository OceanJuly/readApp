...
import { addToShelf, removeFromShelf } from '../../utils/store'
...
export default {
  ...
  methods: {
    ...
    parseBook (url) {
      ...
      // if (this.book.navigation === undefined) {
      //   this.opf = `${process.env.VUE_APP_EPUB_OPF_URL}/${this.fileName}/OEBPS/package.opf`
      //   this.book = new Epub(url)
      // }
      ...
      if (this.fileName) {
        detail({
          ...
          if (response.status === 200 && response.data.error_code === 0 && response.data.data) {
            ...
            // 筛选书籍
            // data[this.categoryText].forEach(item => {
            ...
            // 得到epub中的content.opf文件进行解析
            // this.opf = `${process.env.VUE_APP_EPUB_OPF_URL}/${this.fileName}/OEBPS/content.opf`
            this.opf = `http://localhost:9001/resource/epub/${this.$route.query.category}/${this.$route.query.fileName}.epub`
